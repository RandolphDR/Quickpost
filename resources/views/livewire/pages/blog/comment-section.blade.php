...
<section class="w-full px-4 gap-4 flex flex-col justify-center items-center">
    <div class="w-full flex items-center gap-6 text-gray-400">
        <div class="flex items-center gap-2">
            @auth
                <button wire:click.prevent="toggleLike"
                    class="flex items-center cursor-pointer hover:text-blue-500 transition-colors hover:animate-shake">
                    @if ($isLiked)
        ...
        <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-900 dark:text-gray-400">{{ $post->comments->count() }} Comments</span>
        </div>
    </div>
    ...
    <div class="w-full max-h-96 overflow-x-auto gap-4 flex flex-col justify-start items-center scrollbar-custom">
        @if ($post->comments->isNotEmpty())
            @foreach ($post->comments as $comment)
                <livewire:components.blog.blogpost-comment :commentId="$comment->id" wire:key="comment-{{ $comment->id }}"
                    lazy />
            @endforeach
        @else
