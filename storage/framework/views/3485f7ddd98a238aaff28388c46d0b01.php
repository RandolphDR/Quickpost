<div x-data="{ open: false }" class="relative w-52">
    <form class="w-full flex items-center" @submit.prevent>
        ...
        <input type="text" wire:model.live="query" @focus="open = true" @click.away="open = false" id="search"
            autocomplete="off"
        ...
        <!--[if BLOCK]><![endif]--><?php if(strlen($query) > 1): ?>
            <ul x-show="open" @click.away="open = false" x-transition
                class="absolute top-full left-0 z-30 mt-1 max-h-60 w-full overflow-auto rounded-md border border-gray-300 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800"
                style="display: none;">
